...
const SETTINGS = [
  ...
  { icon: "🎯", label: "Target Band Score", value: "8.5" },
  { icon: "🔔", label: "Notifications", value: "On" },
];

export default function ProfileTab({ onGetPremium }) {
  const [showFullHistory, setShowFullHistory] = useState(false);
  const isPremium = true; // For visual demo
    ...
    <div className="flex flex-col gap-6 pb-24 animate-tab-in">
      ...
      <Card className="flex items-center gap-4 py-8 bg-card-raised border-white/5 relative overflow-hidden group active:scale-[0.99] transition-all cursor-pointer">
         ...
         <div className="flex flex-col gap-1 relative z-10 overflow-hidden">
            <div className="flex items-center gap-2 pr-2">
               <h2 className="text-white font-black text-2xl leading-none truncate pr-1">Mirkomil</h2>
               {isPremium && (
                 <div className="w-5 h-5 bg-accent rounded-full flex items-center justify-center text-white text-[10px] shadow-primary-glow border border-white/20">
                    ✓
                 </div>
               )}
            </div>
            <div className="flex items-center gap-2 mt-2">
               <span className="bg-accent text-white text-[9px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg">Target: 8.5</span>
      ...
      <div className="flex flex-col gap-3">
         ...
         <Card className="p-6 bg-card-raised border-white/5 overflow-visible">
            <div className="h-44 w-full relative flex gap-2">
               {/* Y Axis Labels */}
               <div className="flex flex-col justify-between h-32 text-[10px] text-muted font-bold py-1 w-6 text-right">
                  <span>8.0</span>
                  <span>7.0</span>
                  <span>6.0</span>
                  <span>5.0</span>
               ...
               <div className="flex-1 h-32 relative">
                  {/* Grid Lines */}
                  {[0, 1, 2, 3].map(i => (
                    <div key={i} className="absolute w-full h-[1px] border-t border-dashed border-white/5 z-0" style={{ top: `${(i / 3) * 100}%` }} />
                  ))}
                  
                  <svg viewBox="0 0 400 150" className="w-full h-full relative z-10 overflow-visible">
                     <defs>
                     ...
                     {/* Line */}
                     <path 
                        d="M0,120 L80,105 L160,115 L240,80 L320,60 L400,30" 
                        fill="none" 
                        stroke="#4f8ef7" 
                        strokeWidth="4" 
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        className="drop-shadow-[0_4px_10px_rgba(79,142,247,0.5)]"
                     />
                  ...
                  {/* X Axis Labels */}
                  <div className="flex justify-between w-full mt-2 text-[9px] text-muted font-black uppercase tracking-tighter opacity-60">
                     <span>Mar 1</span>
                     <span>Mar 8</span>
                     <span>Mar 15</span>
                     <span>Mar 22</span>
            ...
            <div className="flex justify-between items-center mt-8 pt-4 border-t border-white/5">
               ...
               <div className="flex flex-col items-end">
                  ...
                  <span className="text-muted text-[10px] font-bold uppercase tracking-widest mt-1 tracking-widest">Improvement</span>
               </div>
            </div>
         </Card>
      </div>

      {/* 3. DETAILED ANALYSIS (FULL WIDTH BARS) */}
      <div className="flex flex-col gap-3">
      ...
      {!isPremium && (
         <Card className="bg-bg border border-gold/40 p-6 flex flex-col gap-4 relative overflow-hidden group active:scale-[0.99] transition-all cursor-pointer" onClick={onGetPremium}>
            <div className="absolute -right-10 -bottom-10 w-40 h-40 bg-gold/5 rounded-full blur-3xl group-hover:scale-125 transition-transform" />
            <div className="relative z-10">
               <span className="text-gold text-[10px] font-black uppercase tracking-widest">🔒 Unlock Full Roadmap</span>
               <h3 className="text-white font-black text-xl leading-tight mt-1">Get Band 8.5 Strategy 👑</h3>
               <p className="text-white/60 text-[13px] font-medium leading-relaxed max-w-[240px] mt-2">Personalized feedback loop based on your latest weaknesses.</p>
            ...
            {SETTINGS.map(s => (
              <button key={s.label} className="w-full flex items-center justify-between px-5 py-5 active:bg-white/5 transition-colors group">
                 ...
                 <div className="flex items-center gap-2 opacity-50">
                    ...
                    <span className="text-muted text-lg leading-none opacity-40">›</span>
                 </div>
              </button>
            ))}
         </Card>
