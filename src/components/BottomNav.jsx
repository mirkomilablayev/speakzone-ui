...
// Icon-only buttons expose aria-label for accessibility
...
const MockIcon = ({ active }) => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill={active ? "currentColor" : "none"} stroke="currentColor" strokeWidth={active ? 0 : 2} aria-hidden="true">
...
const TABS = [
  ...
  { id: "mock",     labelKey: "nav.mock",     Icon: MockIcon     },
  { id: "practice", labelKey: "nav.practice", Icon: PracticeIcon },
...
export default function BottomNav({ activeTab, onTabChange }) {
  ...
  return (
    <nav className="glass-nav flex-shrink-0 flex items-center justify-around" style={{ height: 60 }}
      ...
      {TABS.map(({ id, labelKey, Icon }) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center justify-center gap-1 flex-1 h-full transition-colors duration-200 min-w-0
              ${isActive ? "text-accent" : "text-muted"}`}
          >
            <Icon active={isActive} />
            <span className={`text-[10px] font-medium leading-none truncate ${isActive ? "text-accent" : "text-muted"}`}>
              {label}
            </span>
          </button>
        );
